...
public class TestStreamAPI1 {
	...
	public void lianxi() {
		...
		System.out.println("---------------");
		List<String> strList = Arrays.asList("se", "ee", "re", "vb", "cc", "ee", "ss");
		Stream<String> stream = strList.stream().map(String::toUpperCase);
		stream.forEach(System.out::println);
//		Stream<Stream<Character>> stream1=strList.stream().map(TestStreamAPI1::filterCharacter);
//	    stream1.forEach(e->{
...
//		});
	...
	public void lianxi2() {
		...
		System.out.println("-----------");
	Stream<Employee> stream=emps.stream().sorted((x, y) -> {
			if (x.getAge() == y.getAge()) {
				return x.getName().compareTo(y.getName());
			} else {
				return Integer.compare(x.getAge(), y.getAge());
